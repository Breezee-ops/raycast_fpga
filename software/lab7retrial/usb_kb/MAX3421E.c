...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE reg1 = reg + 2;
	alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg1, 0, NULL, ALT_AVALON_SPI_COMMAND_MERGE);
	int returner = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &val, 0, NULL, 0);
	if(returner < 0){printf("ERROR");}
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE reg1 = reg + 2;
	alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg1, 0, NULL, ALT_AVALON_SPI_COMMAND_MERGE);
	int returner = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes, data, 0, NULL, 0);
	if(returner < 0){printf("ERROR");}
	return (data + nbytes);
}
...
//	reg = reg + 2;
	BYTE val = 0;
	alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 0, NULL, ALT_AVALON_SPI_COMMAND_MERGE);
	int returner = alt_avalon_spi_command(SPI_0_BASE, 0, 0, NULL, 1, &val, 0);
	if(returner < 0){printf("ERROR");}
	return val;
}
//multiple-byte write
...
//	reg = reg + 2;
		alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 0, NULL, ALT_AVALON_SPI_COMMAND_MERGE);
		int returner = alt_avalon_spi_command(SPI_0_BASE, 0, 0, NULL, nbytes, data, 0);
		if(returner < 0){printf("ERROR");}
		return (data + nbytes);
